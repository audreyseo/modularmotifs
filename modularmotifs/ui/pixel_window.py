...
class PixelWindow(abc.ABC):
    ...
    def __init__(
        ...
        self._GLOBAL_GRID = [
            [None for _ in range(self._MAX_WIDTH + self._TKINTER_OFFSET * 2)]
            for _ in range(self._MAX_HEIGHT + self._TKINTER_OFFSET * 2)
        ]
        ...
        style.configure(
            "selected.TLabel", padx=20, pady=20, relief="sunken", borderwidth=3
        )
        style.configure(
            "unselected.TLabel", padx=20, pady=20, relief="flat", borderwidth=3
        )
        self._mode = UIMode.NORMAL
        self._modes = []

        self._selection_img = Image.new(
            mode="RGBA", size=(100, 100), color=(0, 255, 255, 50)
        )
        ...
        self._pixel_canvas = PixelCanvas(
            self._lower_frame, pixel_grid, pixel_size=20, line_width=1.5
        )
        self._pixel_canvas.get_toplevel().pack(
        ...
        save_as_handknit = tk.Button(
            self._controls_frame,
            text="Save as Hand Knit",
            command=self._init_handknit_output(),
        )
        save_as_handknit.pack(side="right", padx=10)

        self._toggle_view_var = tk.StringVar()
        self._toggle_view_var.set(
            "Knit Mode" if self._pixel_canvas.is_knit_mode() else "Grid Mode"
        )
        toggle_view = tk.Button(
            self._controls_frame,
            textvariable=self._toggle_view_var,
            command=self._toggle_view_handler,
        )
        tt = ToolTip(
            toggle_view,
            msg="Toggle pixel view between grid mode and knit mode",
            delay=1.0,
        )
    ...
    def _toggle_view_handler(self):
        self._pixel_canvas.set_mode(
            ViewMode.KNIT if self._pixel_canvas.is_grid_mode() else ViewMode.GRID
        )
        self._toggle_view_var.set(
            "Knit Mode" if self._pixel_canvas.is_knit_mode() else "Grid Mode"
        )
    ...
    def _init_history(self, undo_listener: Callable, redo_listener: Callable) -> None:
        ...
        command_ctrl = (
            self._key_names["Meta"] if PixelWindow.is_mac() else self._key_names["Ctrl"]
        )
    ...
    def _remove_row(
        self, at_index: int, remove_labels: bool = True, add_labels: bool = True
    ) -> None:
