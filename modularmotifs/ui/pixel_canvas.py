...
class PixelCanvas:
    ...
    def __init__(
        self, root: tk.Tk, d: PixelGrid, pixel_size: int = 10, line_width: float = 2.0
    ):
        self.__root = root
        self.__d = d
    ...
    def __init_grid(self):
        ...
        for i in range(self.height() + 1):
            ...
            match self.__mode:
                ...
                case ViewMode.KNIT:
                    self.__hor_lines.append(
                        self.create_knit_horizontal_line(
                            y, fill="#707070", width=self.__line_width
                        )
                    )
            pass
        self.__ver_lines = []
            ...
            for j in range(self.width()):
                ...
                match self.__mode:
                    ...
                    case ViewMode.KNIT:
                        rects_row.append(
                            self.create_knit(
                                x0, y0, needs_multiply=False, fill=c.hex(), width=0
                            )
                        )
                        pass
                pass
    ...
    def event_to_coords(self, event) -> tuple[int, int]:
        if self.__hover_motif:
            simple_x = min(
                self.width() - self.__hover_motif.width(), event.x // self.__pixel_size
            )
            simple_y = min(
                self.height() - self.__hover_motif.height(),
                event.y // self.__pixel_size,
            )
            pass
        else:
    ...
    def set_hover_function(
        self, c: Callable[[int, int], Any], should_reset_hover: bool = True
    ):
    ...
    def set_motif_hover(self):
        def motif_hover(simple_x: int, simple_y: int):
            ...
            self._add_old_id(
                self.create_image(
                    simple_x, simple_y, image=self.__motif_image, anchor="nw"
                )
            )
            self._lower_old_ids()
            pass
    ...
    def create_pixel(self, x0: int, y0: int, **kwargs):
        id = self.__canvas.create_rectangle(
            x0, y0, x0 + self.__pixel_size, y0 + self.__pixel_size, width=0, **kwargs
        )
        if self.__hor_lines:
            self.__canvas.tag_lower(id, self.__hor_lines[0])
            pass
