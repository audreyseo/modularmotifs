...
def rectangular_rgbcolors_to_image(img: Image.Image, w: int, h: int, lol: list[list[RGBAColor]], square_size=1, mode="RGB", opacity=255):
    pixels = img.load()
    if square_size == 1:
        for y in range(h):
            for x in range(w):
...
def list_to_list_of_tuples(shape: list[Any]) -> list[tuple[Any, Any]]:
    assert len(shape) % 2 == 0, f"{list_to_list_of_tuples.__name__}: shape argument has uneven length {len(shape)}"
...
def rgbcolors_to_image(lol: list[list[RGBAColor]], square_size=1, mode="RGB", opacity=255, shape: Union[list[tuple[int, int]], list[int], str]="rect") -> Image.Image:
    ...
    if shape == "rect":
        ...
        return rectangular_rgbcolors_to_image(img, w, h, lol, square_size=square_size, mode=mode, opacity=opacity)
    ...
    for i in range(h):
        ...
        for j in range(w):
            x0 = j * square_size
            draw.polygon([(x0 + x, y0 + y) for x, y in shape_scaled], fill=lol[i][j].rgba_tuple(opacity=opacity) if mode == "RGBA" else lol[i][j].rgb_tuple(), width=0)
            pass
        pass
    return img
...
def motif_to_image(m: Motif, square_size=1, mode="RGB", opacity=255, shape: Union[list[tuple[int, int]], list[int], str]="rect") -> Image.Image:
    return rgbcolors_to_image(motif_to_lol(m), square_size=square_size, mode=mode, opacity=opacity, shape=shape)
...
if __name__ == "__main__":
    ...
    img = motif_to_image(motifs["x-3x3"], square_size=20, mode="RGBA", opacity=127, shape=knit)
