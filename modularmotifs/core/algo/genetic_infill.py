...
def genetic_infill_region_nonuniform(
    ...
    def random_candidate() -> tuple[list[PlacedMotif], _Design]:
        ...
        for y in range(region_height):
            for x in range(region_width):
                motifs_perm = random.sample(
                    motifs_list, num_motifs
                )  # random order of motifs to try
                for m in motifs_perm:
                    if x + m.width() > region_width or y + m.height() > region_height:
                        continue
                    try:
    ...
    def mutate(
        candidate_design: tuple[list[PlacedMotif], _Design], mutation_rate: float = 0.1
    ) -> tuple[list[PlacedMotif], _Design]:
...
def genetic_infill_region(
    ...
    def random_candidate() -> dict:
        return {
            ...
            "grid": [
                [random.randint(0, num_motifs - 1) for _ in range(num_cols)]
    ...
    def simulate(candidate: dict) -> Any:
        ...
        try:
            ...
            float_strands = calculate_float_lengths(
                temp_design, treat_invisible_as_bg=True
            )
            # We need an l_p norm for some p > 1 since sum of float lengths is "constant"
            cost = (sum(len(fs) ** 5 for fs in float_strands)) ** (1.0 / 5.0)
            # print(cost)
    ...
    def crossover(parent1: dict, parent2: dict) -> dict:
        ...
        for r in range(num_rows):
            ...
            child["offsets"].append(
                random.choice([parent1["offsets"][r], parent2["offsets"][r]])
            )
            ...
            if num_cols > 1:
                ...
                row_child = [
                    random.choice([parent1["grid"][r][0], parent2["grid"][r][0]])
                ]
