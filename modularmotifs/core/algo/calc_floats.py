...
def calculate_float_lengths(d: Design, treat_invisible_as_bg: bool = False, selection: Selection = None, min_float_length: int = 1, in_the_round:bool=False) -> Dict[int, int]:
    """
    """
    ...
    def opposite_color(c: Color) -> Color:
        ...
        assert treat_invisible_as_bg, f"calculate_float_lengths: color should not be invisible"
        return Color.FORE
    ...
    for row in d:
        ...
        if lastcolor and (not in_the_round or lastx != -1):
            ...
            # TODO: FIX
            floats.append(FloatStrand(lastx, d.width() - lastx - 1, row=y, color=lastcolor))
            pass
        pass
    return floats
