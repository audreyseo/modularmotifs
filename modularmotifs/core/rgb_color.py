...
class RGBAColor:
    ...
    def from_hex(cls, hexstr) -> Self:
        ...
        assert len(hexstr) == 7 and re.match(
            r"#[0-9a-f]{6}", hexstr, re.I
        ), f"RGBAColor.from_hex: hex string {hexstr} has the wrong format"
    ...
    def rgba_tuple(self, opacity: int = 255) -> tuple[int, int, int, int]:
        ...
        return (
            ...
            self.__green,
            self.__blue,
            self.__alpha if opacity == 255 else opacity,
        )
    ...
    def filtered(
        ...
        return (
            rgba_bounded(self.__red * r),
            rgba_bounded(self.__green * g),
            rgba_bounded(self.__blue * b),
            rgba_bounded(self.__alpha * a),
        )
