...
class Colorization(PixelGrid):
    ...
    def get_color(self, x: int, y: int) -> RGBAColor:
        assert self._d.in_range(
            x, y
        ), f"{self.get_color.__qualname__}: coordinates ({x}, {y}) out of range of design {self._d}"
        pass
    ...
    def swap_colors(self, i: int, j: int) -> Self:
        assert self.in_color_range(i) and self.in_color_range(
            j
        ), f"{self._cname}.swap_colors: color indices {i} and {j} may be out of range of colors {self._colors}"
        temp = self._colors[i]
...
class TwoColorsPerRow(Colorization):
    ...
    def set_fg(self, row: int, c: Union[int, RGBAColor]) -> Self:
        ...
        ), f"{self.set_fg.__qualname__}: row {row} is out of range of design {self._d}"
        assert isinstance(c, RGBAColor) or 0 <= c < len(
            self._colors
        ), f"{self.set_fg.__qualname__}: color index {c} is out of range of color list {self._colors}"
    ...
    def set_bg(self, row: int, c: Union[int, RGBAColor]) -> Self:
        assert (
            0 <= row < self._d.height()
        ), f"{self.set_bg.__qualname__}: row {row} is out of range of design {self._d}"
        assert isinstance(c, RGBAColor) or 0 <= c < len(
            self._colors
        ), f"{self.set_bg.__qualname__}: color index {c} is out of range of color list {self._colors}"
    ...
    def set(
        self, row: int, fg: Union[int, RGBAColor], bg: Union[int, RGBAColor]
    ) -> Self:
    ...
    def complete(self) -> bool:
        ...
        return (
            len(self._foreground) == self._d.height()
            and len(self._background) == self._d.height()
            and all(fg is not None for fg in self._foreground)
    ...
    def get_color(self, x: int, y: int) -> RGBAColor:
        ...
        if not self._treat_invis_as_bg:
            assert (
                c != Color.INVIS
            ), f"{self.get_color.__qualname__}: color of design {self._d} at ({x}, {y}) is invisible but should be either foreground or background"
            pass
    ...
    def to_image(self, square_size=10) -> Image.Image:
        colors = [
            [
                self._foreground[y] if c == Color.FORE else self._background[y]
                for c, x, y in r
            ]
            for r in self._d
        ]
...
class Change:
    ...
    class Permissions(Enum):
        ...
        def from_int(cls, i: int):
            assert (
                Change._perm_min <= i <= Change._perm_max
            ), f"Cannot convert {i} to {cls.__qualname__}"
            match i:
                case 1:
                    return cls.SAME
                case 2:
    ...
    class Necessity(Enum):
        ...
        def from_int(cls, i: int) -> Self:
            assert (
                Change._necc_min <= i <= Change._necc_max
            ), f"Cannot convert {i} to {cls.__qualname__}"
            match i:
                case 1:
                    return cls.MUST
...
class PrettierTwoColorRows(TwoColorsPerRow):
    ...
    ) -> tuple[Optional[RGBAColor], Optional[RGBAColor]]:
    ...
    def _insert_fg_bg(
        self, index: int, fg: Optional[RGBAColor] = None, bg: Optional[RGBAColor] = None
    ):
    ...
    def _remove_fg_bg(
        self, index: int
    ) -> tuple[int, int, Optional[RGBAColor], Optional[RGBAColor]]:
        return (
            self._fg.pop(index),
            self._bg.pop(index),
            self._foreground.pop(index),
            self._background.pop(index),
        )
    ...
    def add_changes(
        ...
        fg: Optional[RGBAColor] = None,
        bg: Optional[RGBAColor] = None,
    ) -> int:
    ...
    def remove_changes(
        self, row: Optional[int] = None
    ) -> tuple[int, Change, Optional[RGBAColor], Optional[RGBAColor]]:
...
if __name__ == "__main__":
    ...
    change = [Change(0, Change.Permissions.from_int(2), Change.Necessity.MUST)] + [
        Change.from_ints(1, 1, 1),
        Change.from_ints(2, 2, 2),
    ]
    print(change)
    print([c.possibilities() for c in change])
    prettier = PrettierTwoColorRows(
        Design(3, 3),
        [
            RGBAColor.from_hex("#FFFF00"),
            RGBAColor.from_hex("#FF00FF"),
            RGBAColor.from_hex("#00FFFF"),
        ],
        change,
    )
    print(prettier)
