...
def test_calculate_float_lengths_with_invisible(design_with_invisible):
    ...
    num = 5
    assert len(result) == num, f"Expected {num} float strands, got {len(result)}: {result}"

    # print(result)
    # assert False
    # Validate float lengths for each strand
    assert len(result[0]) == 1, f"Expected float length of 2, got {len(result[0])}"  # (0, 0) -> (1, 0)
    assert len(result[1]) == 2, f"Expected float length of 2, got {len(result[1])}"  # (0, 1) -> (1, 1)
    assert len(result[2]) == 1, f"Expected float length of 1, got {len(result[2])}"  # (1, 2) -> (2, 2)
    pass
...
def test_calculate_float_lengths_with_selection(design_with_invisible, selection):
    ...
    result = calculate_float_lengths(design_with_invisible, selection=selection, treat_invisible_as_bg=True)
    assert len(result) == 1, f"Expected 1 float strand based on the selection, got {len(result)}: {result}"
    # Check the length of the float
    assert len(result[0]) == 1, f"Expected float length of 1, got {len(result[0])}"

def test_calculate_float_lengths_with_min_float_length(design_with_invisible):
    result = calculate_float_lengths(design_with_invisible, min_float_length=2, treat_invisible_as_bg=True)
    # Expect only floats with length >= 2
    num = 3
    assert len(result) == num, f"Expected {num} float strands with min length 2, got {len(result)}: {result}"
    
    # Ensure the floats have the correct minimum length
    assert all(len(f) >= 2 for f in result), "Not all floats meet the minimum length condition."
